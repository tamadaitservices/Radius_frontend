    ...
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        {!editing ? (
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-green-50 text-green-700 flex-shrink-0">
                  Lv.{level}
                </span>
              </div>
              {(user as any).email && (
                <div className="flex items-center gap-1 text-sm text-gray-500 mt-0.5">
                  <Mail size={13} /><span>{(user as any).email}</span>
                </div>
              )}
              <div className="flex items-center gap-1 text-sm text-gray-500 mt-0.5">
                <Phone size={13} /><span>+91 {user.phone}</span>
              </div>
              {user.role === 'CUSTOMER' && (
          ...
          <div className="space-y-5">
            ...
            {/* Avatar picker with level gates */}
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Choose Avatar</label>
                <span className="text-xs text-gray-400">{level < 20 ? `${purchasesToNextLevel} more purchase${purchasesToNextLevel !== 1 ? 's' : ''} to unlock next` : 'All unlocked!'}</span>
              </div>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: !unlocked ? '#f3f4f6' : editAvatar === av ? AVATAR_BG[av] || '#16a34a' : '#f9fafb',
                      ...
                      title={unlocked ? av : `Unlock at level ${i + 1} (${i * 3} purchases)`}
                    >
                      {!unlocked && (
              ...
              {editAvatar && (
                <button type="button" onClick={() => setEditAvatar('')} className="mt-1.5 text-xs text-gray-400 hover:text-gray-600 underline">
                  Clear avatar
                </button>
              )}
            </div>
      ...
      {level < 20 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-bold text-gray-800">Level {level} → {level + 1}</span>
            <span className="text-xs text-gray-400">{purchasesInCurrentLevel}/3 purchases</span>
          </div>
      ...
      {level === 20 && (
        <div className="bg-green-50 border border-green-100 rounded-2xl p-4 text-center">
          <p className="text-green-700 font-bold text-sm">🏆 Max level reached! All avatars unlocked.</p>
        </div>
      )}
        ...
        {!reservations?.length ? (
          ...
          <div className="space-y-2">
            {reservations.slice(0, 5).map((res: any) => (
              <div key={res.id} className="p-3 rounded-xl hover:bg-gray-50 transition-colors">
                ...
                {/* Show PIN when accepted — customer shows this to vendor */}
                {res.status === 'ACCEPTED' && res.pin && (
                  <div className="mt-2 flex items-center gap-2 bg-green-50 border border-green-100 rounded-xl px-3 py-2">
                    <span className="text-xs text-green-700 font-medium">Show to vendor:</span>
                    <span className="text-xl font-black tracking-[0.3em] text-green-800">{res.pin}</span>
                    <span className="ml-auto text-xs text-green-600">Purchase PIN</span>
                  </div>
                )}
              </div>
            ))}
